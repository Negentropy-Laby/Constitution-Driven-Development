...
`git diff --diff-filter=D --name-only -- .claude/skills .claude/docs README.md UPGRADING.md docs`
currently reports one active deletion:

- `.claude/skills/start/SKILL.md`

Resolution: the old `/start` game onboarding command is preserved in full at
`.claude/docs/reference-archive/start-game-onboarding.md`. It is not restored as
