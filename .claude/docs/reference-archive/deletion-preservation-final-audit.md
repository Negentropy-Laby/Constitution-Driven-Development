...
- Old `/start` is preserved at
  `.claude/docs/reference-archive/start-game-onboarding.md`.
- Historical game GDD content is preserved at
  `.claude/docs/templates/game-design-document.md`.
- Preservation policy is recorded in `.claude/docs/rules-reference.md` and
  `.claude/docs/reference-archive/README.md`.
...
| `.claude/docs/coding-standards.md` | 1 | Gameplay code path wording | Same file retains gameplay/engine rule references under updated CDD context | MIGRATED_IN_PLACE |
| `.claude/docs/director-gates.md` | 28 | CD-GDD and engine/GDD gate names | Same file retains game director gates and rewrites them to CDD / technology terms | MIGRATED_IN_PLACE |
| `.claude/docs/hooks-reference/pre-commit-design-check.md` | 2 | Required GDD section checks | Same hook reference preserves design-doc validation under CDD terminology | MIGRATED_IN_PLACE |
| `.claude/docs/quick-start.md` | 39 | `/start`, GDD, engine, game onboarding text, game-only command descriptions | Same file now routes through `/constitute`; old `/start` archived; game templates, game concepts, and `[游戏专用]` command markers are retained beside product paths | PRESERVED_GAME_SECTION |
| `.claude/docs/rules-reference.md` | 1 | Game rule reference | Same file now explicitly forbids deleting game-specific examples | MIGRATED_IN_PLACE |
| `.claude/docs/skills-reference.md` | 11 | GDD skill descriptions and game workflow examples | Same file retains game workflow entries with CDD terminology and dual-domain notes where applicable | MIGRATED_IN_PLACE |
| `.claude/docs/templates/accessibility-requirements.md` | 3 | Game/system accessibility wording | Same template preserves game accessibility matrix under CDD terminology | MIGRATED_IN_PLACE |
| `.claude/docs/templates/architecture-decision-record.md` | 7 | GDD Requirements Addressed / Engine Compatibility | Same template retains engine compatibility and CDD traceability sections | MIGRATED_IN_PLACE |
| `.claude/docs/templates/architecture-doc-from-code.md` | 1 | GDD reverse-documentation wording | Same template retains reverse-documentation purpose with CDD terminology | MIGRATED_IN_PLACE |
| `.claude/docs/templates/architecture-traceability.md` | 17 | GDD / engine traceability matrix wording | Same template retains traceability, engine risk, and CDD requirement coverage | MIGRATED_IN_PLACE |
| `.claude/docs/templates/collaborative-protocols/design-agent-protocol.md` | 2 | GDD section checklist wording | Same protocol preserves design-agent game section examples with CDD terminology | MIGRATED_IN_PLACE |
| `.claude/docs/templates/collaborative-protocols/implementation-agent-protocol.md` | 2 | GDD / playtest evidence wording | Same protocol preserves implementation evidence rules with CDD terminology | MIGRATED_IN_PLACE |
| `.claude/docs/templates/collaborative-protocols/leadership-agent-protocol.md` | 2 | GDD / gate wording | Same protocol preserves leadership gate semantics with CDD terminology | MIGRATED_IN_PLACE |
| `.claude/docs/templates/concept-doc-from-prototype.md` | 1 | Player Fantasy wording | Same template retains Player Fantasy and playtest-derived concept fields | MIGRATED_IN_PLACE |
| `.claude/docs/templates/difficulty-curve.md` | 4 | game-concept and GDD links | Same game-specific difficulty template now links to CDD paths | MIGRATED_IN_PLACE |
| `.claude/docs/templates/game-concept.md` | 2 | per-system GDD next-step text | Same game-concept template preserves game next steps with CDD wording | MIGRATED_IN_PLACE |
| `.claude/docs/templates/game-design-document.md` | 1 | old game design template replacement | Historical game template restored with Player Fantasy, Game Feel, and playtest acceptance | ARCHIVED |
| `.claude/docs/templates/game-pillars.md` | 2 | game-pillars path text | Same game-specific template retained under CDD path | MIGRATED_IN_PLACE |
| `.claude/docs/templates/hud-design.md` | 3 | GDD / HUD dependency wording | Same game HUD template preserves player/HUD examples with CDD terminology | MIGRATED_IN_PLACE |
| `.claude/docs/templates/player-journey.md` | 3 | game-concept / GDD links | Same player journey template preserves game emotional arc and CDD links | MIGRATED_IN_PLACE |
| `.claude/docs/templates/project-stage-report.md` | 1 | game-concept fixture wording | Same template now accepts game-concept or product-concept | MIGRATED_IN_PLACE |
| `.claude/docs/templates/skill-test-spec.md` | 4 | game-concept / systems-index fixture wording | Same template uses CDD/module-index while preserving game fixture context | MIGRATED_IN_PLACE |
| `.claude/docs/templates/systems-index.md` | 5 | systems-index GDD wording | Superseded by module-index; game source concept and dependency ordering retained | MIGRATED_IN_PLACE |
| `.claude/docs/templates/test-plan.md` | 2 | playtest sign-off wording | Same test template preserves playtest evidence fields | MIGRATED_IN_PLACE |
| `.claude/docs/templates/ux-spec.md` | 2 | GDD UI requirements wording | Same UX template preserves game UI requirement links with CDD terminology | MIGRATED_IN_PLACE |
| `.claude/docs/workflow-catalog.yaml` | 33 | game-only workflow descriptions, team orchestration gate, and GDD paths | Catalog retains game-only steps via `applies_to: [game]` and adds product descriptions or product alternatives beside game descriptions | PRESERVED_GAME_SECTION |
...
| `.claude/skills/start/SKILL.md` | 32 | old game onboarding command | Full file preserved at `.claude/docs/reference-archive/start-game-onboarding.md` | ARCHIVED |
