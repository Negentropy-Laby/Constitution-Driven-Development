...
Use `.claude/docs/workflow-catalog.yaml` as the only required-step source of
truth.
...
> Generated by `/cdd-status` from `.claude/docs/workflow-catalog.yaml`.
> Last updated: [date]
...
Recommend creating it from `.claude/docs/templates/surface-profile.md`.
