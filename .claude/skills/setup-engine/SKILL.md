---
name: setup-engine
description: "Configure the project's technology foundation — game engine or general product stack. Pins the stack in CLAUDE.md, detects knowledge gaps, and populates reference docs via WebSearch when versions are beyond the LLM's training data."
...
Record the framework choice. It drives the CLAUDE.md template, default dependencies, and agent routing.
...
Ask: "May I write these settings to `CLAUDE.md`?"
...
Ask: "May I update the `@` import in `CLAUDE.md` to point to the new reference?"
...
[游戏专用] GDScript/C#/Both variants — CLAUDE.md templates, naming conventions, and specialist routing tables. Referenced only when Godot is the chosen engine.
