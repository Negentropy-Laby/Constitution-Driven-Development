...
This guide covers upgrading your existing game or product project repo from one
version of the template to the next.

> **Current version:** v0.1.0. This template uses `/constitute` as the unified onboarding and
> governance entry for both Game and Product projects. Older release notes may
> mention the historical `/start` command; do not restore it unless you are
> intentionally maintaining a legacy pre-v1.0 template.
...
git remote add template https://github.com/Negentropy-Laby/Constitution-Driven-Development.git
...
changed. Resolve each one — your game/product content stays in place while
structural improvements come along for the ride. Then commit the merge.
...
your engine, stack, and project settings. Keep your content; accept the
structural changes.
...
### Product Project Migration Notes

For general product projects, keep the same command surface as game projects but
merge Product artifacts into the parallel Product paths:

- Concept and module specs: `design/cdd/product-concept.md`,
  `design/cdd/module-index.md`, and Product CDDs in `design/cdd/`
- Stack references: `docs/reference/<stack>/` with version-pinned framework,
  package, SDK, database, or cloud notes
- Architecture: `docs/architecture/architecture.md`, `adr-*.md`,
  `control-manifest.md`, and `tr-registry.yaml`
- UX and validation: `design/ux/`, `design/accessibility-requirements.md`,
  `production/validation/`, `production/qa/evidence/`
- Tests: `tests/contract/`, `tests/cli/`, `tests/e2e/`,
  `tests/migration/`, or stack-appropriate equivalents
...
| **`/brainstorm` update** | Added `Task` to allowed-tools (was missing — blocked all director spawning). Art-director now spawns in parallel with creative-director after pillars lock. Visual Identity Anchor written to concept.md. |
...
| **New feature** | Gate intensity modes: `full` (all director gates), `lean` (phase gates only), `solo` (no directors). In the v1.0 template, set globally via `production/review-mode.txt` during `/constitute`; historical pre-v1.0 templates used `/start`. Override per-run with `--review [mode]` on any gate-using skill |
...
| **Historical `/start` update** | Pre-v1.0 onboarding added Phase 3b — sets review mode during onboarding, writes `production/review-mode.txt`. The v1.0 template uses `/constitute` instead. |
...
.claude/skills/gate-check/SKILL.md
.claude/skills/start/SKILL.md              ← legacy pre-v1.0 only; v1.0 uses .claude/skills/constitute/ and .claude/skills/constitute-check/
...
Set globally during `/constitute` (writes `production/review-mode.txt`). Override any
...
1. Run `/constitute` once to set your preferred review mode — or create `production/review-mode.txt` manually with `full`, `lean`, or `solo`.
...
| Category | Changes |
|----------|---------|
| **New skills (1)** | `/consistency-check` — cross-CDD entity consistency scanner |
...
| **CDD template** | `## Game Feel` section added (input responsiveness, animation targets, impact moments) |
...
.claude/skills/story-readiness/SKILL.md
.claude/skills/story-done/SKILL.md
.claude/skills/design-system/SKILL.md             ← inline CDD skeleton adds Game Feel section
README.md
docs/WORKFLOW-GUIDE.md
UPGRADING.md
```
...
- **`/ux-design`** — guided section-by-section UX spec authoring. Three modes: screen/flow, HUD, or interaction pattern library. Reads CDD UI requirements and player journey. Output to `design/ux/`.
- **`/ux-review`** — validates UX specs against CDD alignment, accessibility tier, and pattern library. Verdict: APPROVED / NEEDS REVISION / MAJOR REVISION.
- **`/team-ui`** updated: Phase 1 now runs `/ux-design` + `/ux-review` as a hard gate before visual design begins.

#### Brownfield Adoption

**`/adopt`** onboards existing projects to the template format. Audits internal structure of CDDs, ADRs, stories, module-index, and infra. Classifies gaps (BLOCKING/HIGH/MEDIUM/LOW). Builds an ordered migration plan. Never regenerates existing artifacts — only fills gaps.
...
`/help` reads your current stage and in-progress work, checks which artifacts are complete, and tells you exactly what to do next — one primary required step, plus optional opportunities. Distinct from `/constitute` (unified onboarding/governance) and `/project-stage-detect` (full audit). Historical pre-v1.0 docs called the first-time-only entry `/start`.

#### Comprehensive QA and Testing Framework

Nine new QA/testing skills covering the full testing lifecycle:

- **`/test-setup`** — scaffolds the test framework and CI/CD pipeline for your engine or product stack
- **`/test-helpers`** — generates engine- or stack-specific test helper libraries (GDUnit4, NUnit, pytest helpers, TypeScript fixtures, etc.)
...
- **`/regression-suite`** — maps test coverage to CDD critical paths, identifies fixed bugs lacking regression tests
...
4. **Run `/adopt`** if you have existing CDDs or ADRs that predate this template version — it will identify which sections need to be added without overwriting your content.
...
**Key themes:** `/design-system` CDD authoring, `/map-systems` rename, custom status line
...
| Category | Changes |
|----------|---------|
| **New skills** | `/design-system` (guided CDD authoring, section-by-section) |
...
| **Skill updates** | `brainstorm`, historical `start`, `design-review`, `project-stage-detect`, `setup-engine` — cross-reference fixes |
...
.claude/skills/gate-check/SKILL.md
.claude/skills/brainstorm/SKILL.md
.claude/skills/start/SKILL.md              ← legacy pre-v1.0 onboarding command
.claude/skills/design-review/SKILL.md
...
| Category | Changes |
|----------|---------|
| **New skills** | Historical `/start` (pre-v1.0 onboarding), `/map-systems` (systems decomposition), `/design-system` (guided CDD authoring). Later v1.0 templates use `/constitute`. |
| **New hooks** | `session-start.sh` (recovery), `detect-gaps.sh` (gap detection) |
| **New templates** | `module-index.md`, 3 collaborative-protocol templates |
...
.claude/skills/map-systems/SKILL.md
.claude/skills/design-system/SKILL.md
.claude/docs/templates/module-index.md
...
- The `## Technology Stack` section (your engine/language or product stack choices)
...
If you ran `/setup-engine`, this file has your engine or product stack config,
naming conventions, and performance budgets. Keep all of it. The template
version is just the empty placeholder.
...
2. **Run `/constitute`** once if you haven't used it — it now correctly identifies
