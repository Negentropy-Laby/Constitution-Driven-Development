...
`.claude/skills/`, `.claude/agents/`, `.claude/hooks/`, `.claude/rules/`, and
`CLAUDE.md` files are GENERATED from the canonical roots by
`scripts/sync_adapters.py --write`. Edit the canonical source, then regenerate;
do not hand-edit these generated copies. Per-directory nested guidance also
generates `CLAUDE.md` siblings under `src/`, `design/`, and `docs/`.
