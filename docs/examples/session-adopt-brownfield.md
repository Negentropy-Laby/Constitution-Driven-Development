...
- `design/` — 3 markdown files with rough design notes (not in CDD template format)
- No `design/cdd/` directory
...
> | Design docs | ⚠️ | 3 files in `design/`, not in CDD format |
> | Architecture docs | ❌ | `docs/architecture/` missing |
> | Systems index | ❌ | `design/cdd/module-index.md` missing |
...
> - Has informal notes, no CDD template sections
> - Contains: some rule descriptions, a damage formula, rough balance numbers
> - Missing from CDD template: Overview, Player Fantasy, Edge Cases, Dependencies, Tuning Knobs, Acceptance Criteria (6 of 8 sections)
>
> *`design/crafting-ideas.md`:*
> - Brainstorm-style notes, no structure
> - No formulas, no acceptance criteria
> - Represents early concept thinking — useful as input for a CDD, not yet a CDD
>
> *`design/inventory.md`:*
> - Closest to a CDD — has Overview and Rules sections
...
> | CDD files not in template format | HIGH | Skills cannot reference TR-IDs from malformed CDDs |
...
> | `design/crafting-ideas.md` — pre-GDD content | LOW | Useful as input; needs full CDD authoring before it blocks anything |
...
> |---|------|-------|----------|-------|
> | 1 | Create `design/cdd/module-index.md` | `/map-systems` or `/adopt` inline | BLOCKING | Unblocks all downstream skills |
> | 2 | Retrofit `design/inventory.md` → CDD format | `/design-system retrofit design/inventory.md` | HIGH | Adds 5 missing sections, keeps your existing content |
> | 3 | Retrofit `design/combat-notes.md` → CDD format | `/design-system retrofit design/combat-notes.md` | HIGH | Adds 6 missing sections |
> | 4 | Convert `design/crafting-ideas.md` → full CDD | `/design-system crafting` (new authoring, uses ideas as input) | MEDIUM | Not a retrofit — too informal to retrofit |
...
**Agent:**
> The BLOCKING gap is the missing `design/cdd/module-index.md`.
>
...
> **Draft systems index:**
>
> | System | Category | Priority | Dependencies | CDD Status |
> |--------|----------|----------|--------------|------------|
...
>
> May I write this to `design/cdd/module-index.md`?
...
**Agent:**
> `design/cdd/module-index.md` created.
>
> **BLOCKING gap resolved.** The following skills can now run:
>
> - ✅ `/design-system [name]` — guided CDD authoring for any system
...
> 3. Run `/design-system crafting` — author crafting CDD using your ideas file as input
>
> After those 3, you'll have 5 of 7 systems with complete CDDs. Then run `/review-all-gdds` to catch cross-system consistency issues before architecture work.
