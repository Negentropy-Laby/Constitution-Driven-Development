...
PHASE 2: SYSTEMS DESIGN
  /map-systems ────────────────────────────────────────────────► design/cdd/module-index.md
        │
        ▼ (for each system, in dependency order)
  /design-system [name] ──────────────────────────────────────► design/cdd/[system].md
  /design-review [system].md ─────────────────────────────────► per-CDD review comments
        │
        ▼ (after all MVP CDDs done)
  /review-all-gdds ────────────────────────────────────────────► design/cdd/cross-review-[date].md
...
PHASE 5: PRODUCTION (repeating sprint loop)
  ...
  /content-audit ──────────────────────────────────────────────► CDD content gaps identified
...
How a single CDD gets authored, reviewed, and handed to architecture:

```
module-index.md (input)
concept.md (input)
upstream CDDs (input, if any)
        │
        ▼
/design-system [name]
        │
        ...
        │
        └── Output: design/cdd/[system].md (complete, all 8 sections)
                │
                ▼
        /design-review design/cdd/[system].md
                │
                ├── APPROVED → mark DONE in module-index, proceed to next system
                ...
                └── MAJOR REVISION → significant redesign needed before next system
                        │
                        ▼ (after all MVP CDDs + cross-review)
                /review-all-gdds
                        │
                        └── Output: cross-review-[date].md
```
...
```
design/cdd/*.md (UI/UX requirements extracted)
...
/story-readiness [story]
        ...
        /dev-story [story]
                │
                ├── Reads: story file, linked CDD requirement, ADR decisions, control manifest
...
/qa-plan [sprint or feature]
        │
        ├── Reads: story files, CDDs, acceptance criteria
...
```
design/cdd/*.md (UX requirements extracted)
...
For projects with existing work (use `/constitute` existing-project path or run directly):
...
/adopt
        ...
        └── Phase 6: fix most urgent gap inline (optional)
                │
                ▼
        /design-system retrofit [path]    → fills missing CDD sections
